...
def HW_Linear(series,s,n):
    ...
    for i in range(0,s):
        L = L + series[m-n+i]
    L = np.divide(L,float(s))
    b = float(0)
    for i in range(0,s):
        b = b + (series[m-n+s+i]-series[m-n+i])
    b = np.divide(float(b),(s*s))
    S = np.zeros(m,float)
    for i in range(0,s):
        S[m-n+i] = np.divide(series[m-n+i],L)
    f = float(0)
    for i in range(m-n+s,m):
        if S[i-s]==float(0):
            S[i-s]=0.1
        p = L
        L = alpha*(np.divide(series[i],S[i-s])) + (1-alpha)*(L+b)
        b = beta*(L-p)+(1-beta)*b
        S[i] = gamma*(np.divide(series[i],L)) + (1-gamma)*S[i-s]
        f = (L + b)*S[i-s]
    return f
...
w4.save('.\Output\Tree\Holt_Winter_M.xls')
